...
/*
    시퀀스 : 자동 번호 발생기
        -- 번호가 자동 발생이 되면 뒤로 되돌릴 수 없다.(삭제후 다시 생성해야 한다.)
...
create sequence dept_seq
    ...
    /* Sequence에 cache를 사용하는 경우/ 사용하지 않는 경우 */
        ...
        -- 서버가 다운된 경우 : 캐쉬된 넘버링이 모두 날라가고 새로운 값(20개 날라가고 다음 값 ex. 1,2,3다운됨 21부터 시작)을 할당 받는다.
...
/*
    INDEX : 테이블의 컬럼에 생성, 특정 컬럼의 검색을 빠르게 사용할 수 있도록 한다.
        ...
        - 테이블 스캔 : 레코드의 처음부터 마지막까지 검색(검색 속도가 느리다.). 인덱스를 사용하지 않고 검색.
        ...
        - 테이블의 검색을 자주하는 컬럼에 Index생성, 테이블 스캔을 하지 않고 Index page를 검색해서 위치를 빠르게 찾는다.
        - Index 를 생성할 때 부하가 많이 걸린다. 주로 업무시간을 피해서 야간에 생성함.
        - Index는 잘 생성해야 한다(오히려 느려질 수 있다). 
*/
...
-- 컬럼에 index가 생성되어 있지 않으면 테이블 스캔을 한다. 첫 레코드부터 하나하나 검색한다.
...
/*
    index는 주기적으로 REBUILD 해줘야 한다. (1주일, 한달)
        - Index Page는 조각난다(Insert, update, delete가 빈번하게 일어나면)
*/

-- Index REBUILD 를 해야하는 정보 얻기 : Index의 Tree 깊이가 4이상인 경우가 조회되면 Rebuild 할 필요가 있다.
...
/*
    Index를 사용해야 하는 경우
        1. 테이블의 행(레코드, 로우)의 갯수가 많은 경우
        2. where 절에서 자주 사용되는 컬럼.
        3. Join 시 사용되는 키 컬럼.
        4. 검색 결과가 원본 테이블 데이터의 2% ~ 4% 정도 되는 경우
        5. 해당 컬럼이 null 이 포함되는 경우(색인은 null은 제외)
        
    Index를 사용하면 안좋은 경우
        1. 테이블의 행의 갯수가 적은 경우
        2. 검색 결과가 원본 테이블의 많은 비중을 차지하는 경우
        3. insert, update, delete가 빈번하게 일어나는 경우
*/
...
create user user_test10 indentified by 1234;    --계정 생성.
