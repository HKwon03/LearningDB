...
select ename, hiredate, to_char(hiredate, 'YY-MON-DD-DY') as 출력결과
from employee;
...
select eno, rpad(substr(eno, 1, 2), length(eno), '*') as "가린번호", ename, rpad(substr(ename, 1, 1), 4, '*') as "가린이름"
from employee;

select eno, rpad(substr(eno, 1, 2), length(eno), '*') as "가린번호", ename, rpad(substr(ename, 1, 1), length(ename), '*') as "가린이름"
from employee;

9. 주민번호 를 출력하되 801210-1****** 출력 하도록, 전화번호 : 010-12******
	dual 테이블 사용
select rpad(substr('801210-1234567',1 , 8), 14, '*') as 주민번호, rpad(substr('010-1211-1111', 1, 6), 13, '*') as "전화번호"
from dual;
