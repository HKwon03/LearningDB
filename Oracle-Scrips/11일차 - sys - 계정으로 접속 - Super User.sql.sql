-- sys - 계정으로 접속 - Super User.sql

-- sys 계정<최고관리자계정>으로 접속 후 특정 테이블에 select 권한 부여하기

-- Autication(인증) : credential(ID + Password)
...
/* with grant option : 특정 계정에 권한을 부여하면서 해당 권한을 다른 사용자에게도 부여할 수 있는 권한 
...
grant select on hr.employee to roletest2;   -- 계정에게 객체 권한을 적용한 것이 아니라 role에 적용.
