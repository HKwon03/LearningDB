...
select ename, distinct dno from employee;   -- 다른 컬럼때문에 오류가 날 수 있다.
...
-- 문자 길이를 출력하는 함수
    -- length : 문자의 길이를 반한, 영문이나 한글 관계없이 글자수를 리턴, 1byte
    
    -- lengthb : 문자의 길이를 반환, 영문 1byte, 한글 3byte 로 반환,
...
-- 문자 조작 함수
  ...
  -- substrb : 문자를 특정 위치에서 잘라오는 함수(영문 1byte, 한글 3byte)
  -- instr : 문자의 특정 위치의 인덱스 값을 반환
  -- instrb : 문자의 특정 위치의 인덱스 값을 반환(영문 1byte, 한글 3byte)
...
-- 이름이 N을 끝나는 사원들 출력하기 (substr 함수를 사용해서)
select ename
from employee
...
-- 87년도 입사한 사원들 출력하기 (substr 함수를 사용해서)
select *
from employee
